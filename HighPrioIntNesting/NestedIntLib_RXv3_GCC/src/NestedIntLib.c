...
{
    R_BSP_SET_INTB((void *)Int_Hook_Vectors);

#if defined(__CCRX__)
    static void dummy_INT_COMMON_NESTEDINT_inline_asm(void);
    dummy_INT_COMMON_NESTEDINT_inline_asm();
#endif
}

// Convert `0xX` format --> `DD` format for CC-RX inline assembler.
#if MY_BSP_CFG_UNNESTED_IPL_MAX >= 16
#error "MY_BSP_CFG_UNNESTED_IPL_MAX should be 0x0 - 0xF."
#elif MY_BSP_CFG_UNNESTED_IPL_MAX == 15
#define _MY_BSP_CFG_UNNESTED_IPL_MAX 15
#elif MY_BSP_CFG_UNNESTED_IPL_MAX == 14
#define _MY_BSP_CFG_UNNESTED_IPL_MAX 14
#elif MY_BSP_CFG_UNNESTED_IPL_MAX == 13
#define _MY_BSP_CFG_UNNESTED_IPL_MAX 13
#elif MY_BSP_CFG_UNNESTED_IPL_MAX == 12
#define _MY_BSP_CFG_UNNESTED_IPL_MAX 12
#elif MY_BSP_CFG_UNNESTED_IPL_MAX == 11
#define _MY_BSP_CFG_UNNESTED_IPL_MAX 11
#elif MY_BSP_CFG_UNNESTED_IPL_MAX == 10
#define _MY_BSP_CFG_UNNESTED_IPL_MAX 10
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  9
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  9
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  8
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  8
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  7
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  7
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  6
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  6
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  5
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  5
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  4
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  4
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  3
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  3
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  2
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  2
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  1
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  1
#elif MY_BSP_CFG_UNNESTED_IPL_MAX ==  0
#define _MY_BSP_CFG_UNNESTED_IPL_MAX  0
#else
#error "MY_BSP_CFG_UNNESTED_IPL_MAX should be 0x0 - 0xF."
#endif
...
#undef IMM
#endif
#define IMM(imm) R_BSP_ASM_TOKEN_SHARP (imm)
#if defined(ASM)
#undef ASM
#endif
#define ASM R_BSP_ASM
#if defined(LAB)
#undef LAB
#endif
#define LAB R_BSP_ASM_LAB
#if defined(LAB_NEXT)
#undef LAB_NEXT
#endif
#define LAB_NEXT R_BSP_ASM_LAB_NEXT
#if defined(LAB_PREV)
#undef LAB_PREV
#endif
#define LAB_PREV R_BSP_ASM_LAB_PREV
/* Miscellaneous. */
#if defined(INT_VECTORS)
#undef INT_VECTORS
#endif
#if defined(STATIC_SYM)
#undef STATIC_SYM
#endif
#if defined(__CCRX__)
#define INT_VECTORS (TOPOF C$VECT)
#define STATIC_SYM(sym_name) __$sym_name
#define U_BSP_ASM_END_NAKED /* none */
#elif defined(__GNUC__)
extern void * const rvectors_start[];
#define INT_VECTORS _rvectors_start
#define STATIC_SYM(sym_name) _##sym_name
#define U_BSP_ASM_END_NAKED );
#elif defined(__ICCRX__)
extern void * const vector_table[];
#define INT_VECTORS _vector_table
#define STATIC_SYM(sym_name) _##sym_name
#define U_BSP_ASM_END_NAKED );
#endif
...
static void dummy_INT_COMMON_NESTEDINT_inline_asm(void)
{
    // To prevent developers from being misled regarding the following inline assemble errors.
    // E0552111:Symbol is undefined
    // E0552113:Symbol definition is not appropriate
    // E0552052:Addressing mode specifier is not appropriate
    // or various inline assemble errors
    return;
}
#endif

R_BSP_PRAGMA_STATIC_INLINE_ASM( INT_COMMON_NESTEDINT_PERMIT )
void INT_COMMON_NESTEDINT_PERMIT(void)
{
R_BSP_ASM_BEGIN
    ...
    ASM( MVFC PSW, R1 )
#ifdef __RXV3
    ASM( BFMOVZ IMM(24), IMM(0), IMM(4), R1, R1 )
#else
    ASM( SHLR IMM(24), R1 )
    ASM( AND IMM(15), R1 )
#endif
    ASM( CMP IMM(_MY_BSP_CFG_UNNESTED_IPL_MAX), R1 )
    ASM( BGEU LAB_NEXT(1) )
    ASM( MVTIPL IMM(_MY_BSP_CFG_UNNESTED_IPL_MAX) )
LAB(1:)
    ASM( SETPSW I )

    ASM( MOV.L 4[R0], R1 ) // address of hook_entry_XXX + 3 --> R1 // 3 = size of BSR.W
    ASM( ADD IMM(INT_VECTORS - (STATIC_SYM( hook_entry___0 ) + 3)), R1 ) // 3 = size of BSR.W
    ASM( MOV.L [R1], 4[R0] )
    ASM( RTSD R_BSP_ASM_TOKEN_SHARP 4, R1-R1 ) // i.e. POP R1, JMP [SP+]

U_BSP_ASM_END_NAKED
}

R_BSP_PRAGMA_STATIC_INLINE_ASM( INT_COMMON_NESTEDINT_NOT_PERMIT )
void INT_COMMON_NESTEDINT_NOT_PERMIT(void)
{
R_BSP_ASM_BEGIN
    ...
    ASM( ADD IMM(INT_VECTORS - (STATIC_SYM( hook_entry___0 ) + 3)), R1 ) // 3 = size of BSR.W
    ASM( MOV.L [R1], 4[R0] )
    ASM( RTSD IMM(4), R1-R1 ) // i.e. POP R1, JMP [SP+]

U_BSP_ASM_END_NAKED
}

#if defined(__CCRX__)
R_BSP_PRAGMA_STATIC_INLINE_ASM( INT_ENTRY_NESTEDINT_PERMIT )
#else
R_BSP_PRAGMA_STATIC_INLINE( INT_ENTRY_NESTEDINT_PERMIT )
#endif
void INT_ENTRY_NESTEDINT_PERMIT(void)
{
R_BSP_ASM_BEGIN

    ASM( BSR.W STATIC_SYM( INT_COMMON_NESTEDINT_PERMIT ) )

U_BSP_ASM_END_NAKED
}

#if defined(__CCRX__)
R_BSP_PRAGMA_STATIC_INLINE_ASM( INT_ENTRY_NESTEDINT_NOT_PERMIT )
#else
R_BSP_PRAGMA_STATIC_INLINE( INT_ENTRY_NESTEDINT_NOT_PERMIT )
#endif
void INT_ENTRY_NESTEDINT_NOT_PERMIT(void)
{
R_BSP_ASM_BEGIN

    ASM( BSR.W STATIC_SYM( INT_COMMON_NESTEDINT_NOT_PERMIT ) )
...
// Documents
// English
// https://www.renesas.com/us/en/document/mas/rx-family-rxv1-instruction-set-architecture-users-manual-software-rev130
// https://www.renesas.com/us/en/document/mas/rx-family-rxv2-instruction-set-architecture-users-manual-software
// https://www.renesas.com/us/en/document/mas/rx-family-rxv3-instruction-set-architecture-users-manual-software-rev100
// Japanese
// https://www.renesas.com/jp/ja/document/mas/rx-family-rxv1-instruction-set-architecture-users-manual-software-rev130
// https://www.renesas.com/jp/ja/document/mas/rx-family-rxv2-instruction-set-architecture-users-manual-software
...
void * const dummy_INT_COMMON_NESTEDINT_PERMIT = (void *)INT_COMMON_NESTEDINT_PERMIT;
void * const dummy_INT_COMMON_NESTEDINT_NOT_PERMIT = (void *)INT_COMMON_NESTEDINT_NOT_PERMIT;
...
// To prevent the symbol from being optimized out.
static void INT_COMMON_NESTEDINT_PERMIT(void) __attribute__((used));
static void INT_COMMON_NESTEDINT_NOT_PERMIT(void) __attribute__((used));

#elif defined(__ICCRX__)

// To prevent the symbol from being optimized out.
#pragma required=INT_COMMON_NESTEDINT_PERMIT
#pragma required=INT_COMMON_NESTEDINT_NOT_PERMIT
#pragma required=vector_table

#endif
// The code size of the following functions is just 4 bytes.
// For PERMIT
// 00000000 39rrrr               W  BSR.W STATIC_SYMBOL( INT_COMMON_NESTEDINT_PERMIT )
// 00000003 02                      RTS // Not used.
// For NOT_PERMIT
// 00000000 39rrrr               W  BSR.W STATIC_SYMBOL( INT_COMMON_NESTEDINT_NOT_PERMIT )
// 00000003 02                      RTS // Not used.

#define _INT_ENTRY(sel) INT_ENTRY_NESTEDINT_##sel()
