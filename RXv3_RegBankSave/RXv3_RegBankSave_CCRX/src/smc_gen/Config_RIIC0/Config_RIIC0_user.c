...
#define r_Config_RIIC0_transmit_interrupt(...) R_CG_ATTRIB_INTERRUPT_EI(r_Config_RIIC0_transmit_interrupt, __VA_ARGS__)
#define r_Config_RIIC0_receive_interrupt(...)  R_CG_ATTRIB_INTERRUPT_EI(r_Config_RIIC0_receive_interrupt,  __VA_ARGS__)
...
//#pragma interrupt __r_Config_RIIC0_transmit_interrupt(vect=VECT(RIIC0,TXI0), bank=0)
#pragma inline_asm _r_Config_RIIC0_transmit_interrupt
...
R_BSP_PRAGMA_STATIC_INTERRUPT(_r_Config_RIIC0_transmit_interrupt, VECT(RIIC0,TXI0))

static void _r_Config_RIIC0_transmit_interrupt(void)
{
#if 0
    MVFC PSW, R1
    BFMOVZ #18H, #00H, #04H, R1, R1
    CMP #0AH, R1
    BGTU.B ?+
//    MVTIPL #0BH
    MVTIPL # BSP_CFG_FIT_IPL_MAX
?:
    ...
    RSTR #00H
    BRA.W __$r_Config_RIIC0_transmit_interrupt
}

#if 0
static void __r_Config_RIIC0_transmit_interrupt(void)
{
    if(BSP_CFG_FIT_IPL_MAX > (((R_BSP_GET_PSW()) >> 24) & 0xF))
    {
        R_BSP_SET_IPL(BSP_CFG_FIT_IPL_MAX);
    ...
    _r_Config_RIIC0_transmit_interrupt();
}
#endif
